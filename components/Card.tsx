...
const Card = ({ image, title, subTopics }: Props) => {
  return (
    <Link
      href="job/[...metadata]"
      as={`/job${image}/${title}/${subTopics}?name="bhushan"`}
    >
      <div
        className="rounded-r-rounded-t-2xl mt-5 w-full cursor-pointer rounded-t-2xl  bg-white
    shadow-lg hover:shadow-2xl sm:w-[45%] lg:w-[30%] xl:w-[20%]"
      >
        ...
        <div className="">
          ...
          <div
            className="my-2 mx-3 flex flex-wrap space-x-3 pt-3 pb-8 text-sm
            leading-10"
          >
            {subTopics.map((i, num) => {
              return (
